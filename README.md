...
A Chrome extension that helps you browse with clarity: scrape and score article links, automate page actions with natural language, and speak commands using voice (OpenAI Whisper).
...
- **Voice** – Use the microphone button to speak a command; audio is sent to the backend, transcribed with OpenAI Whisper, and the text is filled into the command box. Click **Run** (or press Enter) to execute.
...
│   ├── routes/              # automate, scrape, voice (Whisper)
...
3. **Environment**
   ...
   - Fill in at least:
     ...
     - `OPENAI_API_KEY` (optional: ad removal + **voice transcription**)
