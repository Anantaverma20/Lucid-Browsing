...
def create_writer() -> LlmAgent:
    return LlmAgent(
        ...
        instruction=(
            ...
            "For hide/remove: pick the selectors that best match what the user asked (e.g. 'weather card' -> element with 'weather' in id/class or text; 'sidebar' -> element with sidebar in id/class). "
            ...
            "- Click: check element exists before acting (if (el) el.click();). "
            "- REMOVE/HIDE: use only selectors from page_context (ids and classes listed there). Remove or set style.display='none' on each match. "
            "Task spec: {task_spec}. "
